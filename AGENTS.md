...
- Watts transcript artifacts are intentionally written to `.local-corpus/watts/`, also gitignored; public generated artifacts live under `watts/`.
...
- `scripts/build-brensilver-feed.py` seeds from existing `brensilver/talks.json` when present, so historical archived talks are preserved if an upstream source feed shrinks.
...
- Preferred one-command local ingestion: `scripts/run-brensilver-ingestion.sh`. It refreshes source feeds first, then runs `run-corpus`. Set `BRENSILVER_AUTO_PUBLISH=1` only for a scheduled job that should commit and push generated `brensilver/` artifacts automatically.
...
PYTHONPATH=tools/brensilver-transcripts/src python3 -m brensilver_transcripts.pipeline --corpus-config tools/brensilver-transcripts/config/watts-corpus.json run-corpus --limit 50 --feed-every 10 --media-base-url https://jeffharr.is/watts/ --copy-artwork --update-qmd --build-feedback-viewer
```
